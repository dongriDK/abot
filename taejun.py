        ...
        for member in guild.members:
            ...
            if (member.bot != True):
                ...
                if (ttext == 0 and ttime == 0):
                    ...
                    if (datetime.datetime(cur_year, curday[0], curday[1]) - datetime.datetime(cur_year, jointime1[0], jointime1[1]) < 15):
        ...
        for member in guild.members:
            ...
            if (member.bot != True):
                ...
                if (ttime > 0 and ttime < 7200):
                    ...
                    if (datetime.datetime(int(cur_year), int(curday[0]), int(curday[1])) - datetime.datetime(int(cur_year), int(jointime1[0]), int(jointime1[1])) < 15):
                        newjoinList.append(member.mention + " (" + member.name + "#" + member.discriminator + ") " + " ㅤ**" + jointime + "**\n")
                    else:
                        chat = ""
                        chat += member.mention
